...
public class RandomUtils {
    ...
    /**
     * Returns a random real number from a Gaussian distribution with mean &mu;
     * and standard deviation &sigma; AND within the given range.
     *
     * @param mu    the mean
     * @param sigma the standard deviation
     * @param
     ...
     * with mean {@code mu} and standard deviation {@code sigma}
     */
    public static double gaussian(Random random, double mu, double sigma, double lowerBound, double upperBound) {
        double toReturn = Integer.MIN_VALUE;
        while ((toReturn < lowerBound) || (toReturn > upperBound)) {
            toReturn = mu + sigma * gaussian(random);
        }
        return toReturn;
    }
